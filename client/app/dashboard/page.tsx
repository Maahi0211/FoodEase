...
export default function MenuPage() {
  ...
  const fetchDishes = async () => {
    try {
      const response = await axios.get('https://foodease-1.onrender.com/api/dish/all-dishes', {
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post<DishResponse>('https://foodease-1.onrender.com/api/dish/add-dish', {
        name: formData.name,
        description: formData.description,
        price: parseFloat(formData.price),
        dishUrl: formData.dishUrl,
  ...
  const confirmDelete = async () => {
    try {
      ...
      const response = await axios.delete(`https://foodease-1.onrender.com/api/dish/remove/${deleteModal.dishId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
